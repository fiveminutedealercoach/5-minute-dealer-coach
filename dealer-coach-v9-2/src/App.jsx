  ...
  const submit = async () => {
    ...
    if(livePhase === 'live' && activeS) {
      ...
      const personaFallbacks = {
        dave:'Are you going to match that price or not?',
        linda:'I still need more time to think about this.',
        mike:'That payment is still too high for me.',
        gary:'I know what my truck is worth.',
        carol:'Let me just check online first.',
        frank:'How do I know this repair is really necessary?',
        barbara:'That price is way more than I expected.',
        ray:'My mechanic has always taken good care of me.',
        susan:'I need to call my husband before approving anything.',
        tom:'I am just not ready to do this today.'
      }
      ...
      try {
        const res = await fetch('/ai-proxy', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            system: 'You are ' + persona.name + ', a real automotive customer. Personality: ' + persona.tone + '. Concern: ' + persona.desc + '. Escalation: ' + persona.escalation + '. Exchange ' + newExCount + ' about: ' + activeS.objection.replace(/"/g,'') + '. Respond in 1-2 spoken sentences. React to what was just said. Never repeat yourself. Add [CLOSE_EARNED] only if rep genuinely earned it after exchange 2.',
        ...
        if (!rawReply || rawReply.length < 5) throw new Error('Empty')
        speakReply(rawReply)
      } catch(e) {
        speakReply(personaFallbacks[persona?.id] || 'I hear you but I am still not convinced.')
      }
      return
    }
...
Your next reaction as ${persona.name}:`}]
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    // Per-persona hardcoded openers as fallback if API is unavailable
    const hardcodedOpeners = {
      dave:    'I found this exact vehicle online for $2,400 less. Are you going to match that price or not?',
      linda:   'Look, I got burned badly at my last dealership. I need to know exactly what I am getting into before I commit to anything.',
      mike:    'I need to be honest with you  -  I can only do $450 a month. That is my absolute ceiling.',
      gary:    'I have been offered $28,000 for my truck everywhere else. I am not taking a dollar less.',
      carol:   'Before we go any further, walk me through every single fee on this deal. I want to see everything.',
      frank:   'Every time I bring my car in here something new comes up. How do I know this repair is actually necessary?',
      barbara: 'That estimate is almost double what I was expecting. I could get this done way cheaper somewhere else.',
      ray:     'I appreciate your time but my mechanic Tony has been working on my cars for fifteen years. I really trust him.',
      susan:   'I like everything about this car but I never make a decision this big without talking to my husband first.',
      tom:     'I hear you but honestly right now just is not the right time for me to do this.'
    }

    // Start drill immediately with hardcoded opener, upgrade to AI opener if available
    const fallbackOpener = hardcodedOpeners[persona.id] || 'I have some concerns before I can move forward.'
    ...
    try {
      const res = await fetch('/ai-proxy', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          system: 'You are ' + persona.name + ', a real automotive customer. Personality: ' + persona.tone + '. Your concern: ' + persona.desc + '. State your objection naturally in 1-2 spoken sentences.',
          messages: [{ role: 'user', content: 'State your objection about: ' + script.objection.replace(/"/g,'') }],
          max_tokens: 80
        })
      })
      const data = await res.json()
