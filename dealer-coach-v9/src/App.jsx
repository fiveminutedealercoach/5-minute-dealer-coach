...
function TrackDash({results,onRemove,onLog,preloadScript,dealer}) {
  ...
  // Open log sheet if coming from preloadScript
  useEffect(()=>{if(preloadScript)setShowLog(true)},[preloadScript])
  ...
  if(isMgr) {
    ...
    return(
      <div style={{padding:'16px 16px 100px',position:'relative'}}>
        ...
        {/* Floating + Log button */}
        <div onClick={()=>setShowLog(true)} style={{position:'fixed',bottom:82,right:'max(16px, calc(50vw - 224px))',width:52,height:52,borderRadius:'50%',background:C.green,display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,cursor:'pointer',boxShadow:`0 4px 20px rgba(184,255,60,0.4)`,zIndex:200}}>+</div>
      </div>
    )
  }
