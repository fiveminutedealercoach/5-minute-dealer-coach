...
Your next reaction as ${persona.name}:`}]
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    try {
      ...
      // ── SEND-WHEN-DONE APPROACH ─────────────────────────
      // Rep speaks freely, taps Send when done
      // OpenAI only receives audio on explicit commit  -  AI can never cut itself off
      let aiSpeaking       = false
      let recordingStarted = false
      const sendEvent = (obj) => { if (dc && dc.readyState === 'open') dc.send(JSON.stringify(obj)) }

      // Expose controls to React state via refs
      const sendResponseRef_inner = () => {
      ...
      sendResponseRef.current = sendResponseRef_inner

      dc.onopen = () => {
        setLivePhase('live')
        setLiveStatus('')
        const voice = REALTIME_VOICES[persona.id] || 'alloy'
        // Set voice + transcription only  -  never override persona instructions
        dc.send(JSON.stringify({
          type: 'session.update',
          session: {
            voice,
            input_audio_transcription: { model: 'whisper-1' },
            turn_detection: null, // WE control when to send
      ...
      dc.onmessage = e => {
        try {
          ...
          if (evt.type === 'response.created') {
            ...
            recordingStarted = false
            setLiveRecording(false)
            setLiveStatus('') // clear any status
          }

          // AI done generating  -  tell rep it is their turn
          if (evt.type === 'response.done') {
            setTimeout(() => {
              aiSpeaking = false
              recordingStarted = true
              setLiveRecording(true) // show Send button
              setLiveStatus('Your turn  -  speak then tap Send Response')
            }, 600)
          }
          ...
          if (aiText && aiText.trim().length > 3) {
            setLiveTranscript(prev => {
              if (prev.length > 0 && prev[prev.length-1].text === aiText) return prev
              const next = [...prev, { role: 'customer', text: aiText }]
          ...
          if (repText && repText.trim().length > 2) {
            setLiveTranscript(prev => {
              const next = [...prev, { role: 'rep', text: repText }]
              liveTranscriptRef.current = next
              setTimeout(()=>{ const b=document.getElementById('live-transcript-box'); if(b) b.scrollTop=b.scrollHeight }, 100)
              return next
            })
            // Count exchange when rep speaks
          ...
          // Errors
          if (evt.type === 'error') {
            setLiveError('Session error  -  ' + (evt.error?.message || 'Unknown'))
          }
  ...
  if((livePhase==='connecting'||livePhase==='live'||livePhase==='ended')&&activeS) {
    ...
    return(
      <div style={{padding:'16px 16px 80px'}}>
        ...
        {/* SEND RESPONSE  -  appears when it is rep's turn */}
        {livePhase==='live'&&liveRecording&&(
          <button onClick={()=>sendResponseRef.current&&sendResponseRef.current()} style={{width:'100%',background:C.green,color:C.navy,fontFamily:fH,fontWeight:900,fontSize:17,letterSpacing:1,textTransform:'uppercase',padding:18,borderRadius:10,border:'none',cursor:'pointer',marginBottom:10,boxShadow:'0 0 40px rgba(184,255,60,0.5)'}}>
            🎤 Send Response
          </button>
        )}

        {/* Waiting  -  AI is speaking */}
        {livePhase==='live'&&!liveRecording&&(
          <div style={{width:'100%',background:'rgba(26,107,255,0.08)',border:`1px solid rgba(26,107,255,0.2)`,borderRadius:10,padding:14,marginBottom:10,textAlign:'center'}}>
            <div style={{fontFamily:fH,fontSize:12,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.blueBright}}>
              {liveTranscript.length===0?'Connecting...':`${PERSONAS.find(p=>p.id===activePersId)?.name||'AI'} is speaking...`}
            </div>
            <div style={{fontSize:11,color:C.gray,marginTop:4}}>Send Response button appears when done</div>
          </div>
        )}
