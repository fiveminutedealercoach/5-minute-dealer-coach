...
One coaching whisper:`}],
  ...
  const submit = async () => {
    ...
    // ── LIVE DRILL MODE  -  use live transcript system ────────
    if(livePhase === 'live' && activeS) {
      ...
      // Add rep response to live transcript
      ...
      if (newExCount >= 5) {
        setTimeout(() => endLiveDrill(activeS, liveTranscriptRef.current), 1000)
        return
      }

      // Get next persona response via Claude
      const persona = PERSONAS.find(p => p.id === activePersId) || getPersonaForScript(activeS)
      const silentTip = getSilentCoach(repText, newExCount - 1, activeS)

      try {
        const fullConvo = updatedLive.map(t =>
          (t.role === 'rep' ? 'Rep' : persona.name) + ': "' + t.text + '"'
        ).join(' | ')
        ...
        const res = await fetch('/ai-proxy', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            system: `You are ${persona.name}. A real person in a dealership. Core concern: ${persona.desc}. Your emotional register: ${persona.tone}. Escalation: ${persona.escalation}.

Exchange ${newExCount} of conversation about "${activeS.objection.replace(/"/g,'')}"

RULES:
- 1-2 sentences MAX. Natural spoken language only.
- React to what they SPECIFICALLY said  -  not a generic pushback
- Never repeat yourself
- Exchanges 1-2: Hold firm. Exchanges 3-4: Soften slightly only if they addressed your concern directly. Exchange 5: Either agree or be done.
- If the rep gave a strong Acknowledge + specific value + direct close question after exchange 2, include [CLOSE_EARNED] at the end.
- Sound like a real ${persona.tone} person.`,
            messages: [{ role: 'user', content: `Full conversation: ${fullConvo}

Your next response as ${persona.name}:` }],
            max_tokens: 80
          })
        })
        const data = await res.json()
        let reply = data.content?.[0]?.text?.trim() || persona.desc
        const closeEarned = reply.includes('[CLOSE_EARNED]')
        reply = reply.replace('[CLOSE_EARNED]', '').trim()

        // Add persona reply to live transcript
        const withReply = [...liveTranscriptRef.current, { role: 'customer', text: reply }]
        setLiveTranscript(withReply)
        liveTranscriptRef.current = withReply
        setAiText(reply)
        setLiveStatus('Speaking...')

        // Speak via ElevenLabs then auto-record
        ...
        setLiveStatus('Connection issue  -  tap Send Now to retry')
        setLiveRecording(true)
      }
      return  // exit  -  don't run regular drill logic below
    }
...
Your next reaction as ${persona.name}:`}]
  ...
  // ══════════════════════════════════════════════════════════
  // ══════════════════════════════════════════════════════════
  // VOICE DRILL ENGINE  -  ElevenLabs + Claude + Auto-Record
  // Persona speaks via ElevenLabs TTS, mic auto-opens when done,
  // silence detection auto-submits after 1.5s, zero cut-off possible
  // ══════════════════════════════════════════════════════════

  // Auto-record after persona finishes speaking
  const autoStartRecording = () => {
    setLiveStatus('Listening...')
    setLiveRecording(true)
    accumulatedRef.current = ''
    setTranscript('')
    // Start mic with silence detection
    try {
      const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition
      if (!SpeechRecognition) { setLiveRecording(false); return }
      const rec = new SpeechRecognition()
      rec.continuous    = true
      rec.interimResults = false  // final only  -  no repetition loops
      rec.lang          = 'en-US'
      let lastResultIndex = 0
      ...
      rec.onerror = (e) => {
        ...
        setLiveRecording(false)
        setLiveStatus('Mic error  -  tap Send to submit manually')
      }

      recRef.current = rec
      rec.start()
      // Safety timeout  -  max 30s recording
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    // Get persona opening line from Claude
    try {
      const res = await fetch('/ai-proxy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          system: `You are ${persona.name}, a ${persona.tone} automotive customer. Your core concern: ${persona.desc}. Your escalation pattern: ${persona.escalation}.

State your objection naturally in 1-2 sentences as if you just walked into the dealership. Sound like a real person  -  not a script. Objection context: "${script.objection}"`,
          messages: [{ role: 'user', content: 'Start the conversation by stating your objection.' }],
          max_tokens: 80
        })
      })
      const data = await res.json()
      const opening = data.content?.[0]?.text?.trim() || `I need to be honest with you  -  ${script.objection.replace(/"/g,'')}.`

      // Add to transcript
      const firstExchange = [{ role: 'customer', text: opening }]
      setLiveTranscript(firstExchange)
      liveTranscriptRef.current = firstExchange
      setAiText(opening)
      setLivePhase('live')
      setLiveStatus('Speaking...')
      ...
      speak(opening, () => {
        setSpeaking(false)
        // Auto-start recording after 500ms pause
        setTimeout(() => autoStartRecording(), 500)
      }, pVoice)

    } catch(e) {
      setLivePhase('idle')
      setLiveError('Could not start drill  -  check connection and try again.')
    }
  }
  ...
  const endLiveDrill = async (script, transcriptSnapshot) => {
    ...
    const transcript = transcriptSnapshot || liveTranscriptRef.current
    ...
    const lastRepLine = repLines[repLines.length - 1] || repLines[0] || ''
    ...
    if (repLines.length === 0) {
      setLivePhase('idle')
      setLiveError('No responses captured. Speak clearly after the persona finishes.')
