...
One coaching whisper:`}],
  ...
  const submit = async () => {
    ...
    const MAX_EXCHANGES = 8  // max before auto-coaching

    setLoading(true)
    const persona = PERSONAS.find(p=>p.id===activePersId)||getPersonaForScript(activeS)

    // Detect confidence markers in rep response
    const hesitationWords = ['i think','maybe','possibly','i'm not sure','i guess','kind of','sort of','probably','might']
    ...
    try{
      ...
      const res = await fetch('/ai-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        system:`You are ${persona.name}, a ${persona.tone} automotive customer in a real dealership conversation. Your core concern: ${persona.desc} Your escalation pattern: ${persona.escalation}

RULES:
- Respond in 1-2 short spoken sentences only — natural conversation
- Stay emotionally consistent: ${persona.tone}
- Never break character, never coach, never summarize
- If the salesperson gave a strong Acknowledge + specific value pivot + clear close question, you may say something like "Okay, that makes sense. Let me think about that." — this signals the rep has earned the close
- If they were weak, generic, or didn't close, push back harder
- Sound like a real person, not a script

CLOSE SIGNAL: If you are ready to move forward, include the exact phrase [CLOSE_EARNED] at the very end of your response (hidden from customer — for system use only). Only use this after exchange 3+ if the rep has genuinely earned it.`,
...
Your next reaction as ${persona.name}:`}]
      ...
      const data = await res.json()
      let reply = data.content?.[0]?.text||`I hear you but I'm still not convinced.`
    ...
    return(
      <div style={{padding:'16px 16px 80px'}}>
        ...
        <div style={{background:'linear-gradient(135deg,rgba(184,255,60,0.08),rgba(184,255,60,0.03))',border:'1px solid rgba(184,255,60,0.25)',borderRadius:12,padding:16,marginBottom:14}}>
          ...
          {/* Word track comparison — your words vs model */}
          {allTranscripts.length>0&&activeS?.script&&(
            <div style={{background:'rgba(255,255,255,0.03)',border:`1px solid ${C.border}`,borderRadius:10,padding:'12px 14px',marginBottom:12}}>
              <div style={{fontFamily:fH,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:C.gray,marginBottom:10}}>📊 Your Words vs Model</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                <div>
                  <div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.yellow,marginBottom:6}}>You Said</div>
                  {allTranscripts.map((t,i)=>(
                    <div key={i} style={{background:'rgba(255,201,71,0.06)',border:'1px solid rgba(255,201,71,0.15)',borderRadius:6,padding:'6px 8px',marginBottom:6,fontSize:11,color:C.lightText,lineHeight:1.5,fontStyle:'italic'}}>"{t}"</div>
                  ))}
                </div>
                <div>
                  ...
                  <div style={{background:'rgba(184,255,60,0.06)',border:'1px solid rgba(184,255,60,0.2)',borderRadius:6,padding:'6px 8px',fontSize:11,color:C.white,lineHeight:1.5,fontStyle:'italic'}}>"{activeS.script}"</div>
                  <div style={{background:'rgba(184,255,60,0.04)',border:'1px solid rgba(184,255,60,0.1)',borderRadius:6,padding:'6px 8px',marginTop:4,fontSize:11,color:C.gray,lineHeight:1.5,fontStyle:'italic'}}>Follow-up: "{activeS.followup}"</div>
                </div>
              </div>
            </div>
          )}
