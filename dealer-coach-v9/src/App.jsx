  ...
  const submit = async () => {
    ...
    if(newEx<3){
      ...
      try{
        ...
        const res = await fetch('/ai-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
          system:`You are ${persona.name}, a ${persona.tone} automotive customer in a real dealership conversation. Your core concern: ${persona.desc} Your escalation style: ${persona.escalation}

STRICT RULES:
- Respond in 1-2 short spoken sentences only — natural conversation, not a speech
- Stay emotionally consistent with your character — ${persona.tone}
- Never repeat the objection title or any label. Never say what your objection "is"
- Never break character, never coach, never summarize
- If the salesperson handled it well, soften slightly but don't fully give in yet
- If they were weak or generic, push back harder using your escalation pattern
- Sound like a real person talking, not a script`,
          messages:[{role:'user',content:`Context: The conversation is about "${activeS.objection.replace(/"/g,'')}" in ${activeS.dept}.
What you just said: "${aiText}"
What the salesperson said: "${transcript}"
Your next spoken reaction as ${persona.name} (1-2 sentences, natural speech only):`}]
        })})
        const data = await res.json()
