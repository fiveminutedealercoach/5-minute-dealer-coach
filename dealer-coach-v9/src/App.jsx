...
Your next reaction as ${persona.name}:`}]
        ...
        try {
          ...
          if (evt.type === 'response.audio_transcript.done') {
            if (evt.transcript && evt.transcript.trim().length > 3) {
              ...
              // Exchange count only increments after REP speaks — not here
            }
          }
  ...
  const endLiveDrill = async (script, transcriptSnapshot) => {
    ...
    if (transcript.length > 0 && targetScript) {
      ...
      // No transcript — back to list gracefully
      clearTimeout(safetyTimer)
      setLivePhase('idle')
      setLiveTranscript([])
      setLiveStatus('')
            ...
            <div key={s.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:'12px 14px',display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{display:'flex',gap:6,flexShrink:0}}>
                <button onClick={()=>{
                  ...
                  startLiveDrill(s,p)
                }} style={{background:C.green,color:C.navy,fontFamily:fH,fontWeight:900,fontSize:12,letterSpacing:1,textTransform:'uppercase',padding:'10px 12px',borderRadius:8,border:'none',cursor:'pointer'}}>GO</button>
                <button onClick={()=>{
                  const p=getPersonaForScript(s)
                  setActiveS(s); setActivePersId(p.id)
                  setFeedback(null); setConfidenceFlags([]); setCloseEarnedFlag(false)
