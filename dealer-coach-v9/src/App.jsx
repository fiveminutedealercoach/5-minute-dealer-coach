...
Your next reaction as ${persona.name}:`}]
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    try {
      // Generate persona opening line
      const res = await fetch('/ai-proxy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          system: `You are ${persona.name}, a real automotive customer. Personality: ${persona.tone}. Your concern: ${persona.desc}.
State your objection naturally in 1-2 spoken sentences. Sound like a real person walking into a dealership  -  not a script.`,
          messages: [{ role: 'user', content: `Your objection is about: "${script.objection.replace(/"/g,'')}". State it naturally:` }],
          max_tokens: 80
        })
      })
      const data = await res.json()
      ...
      // Add to transcript
      const first = [{ role: 'customer', text: opening }]
      ...
      // Speak via ElevenLabs  -  auto-start recording when done
      const pVoice = getPersonaVoiceOpts(persona)
      setSpeaking(true)
      speak(opening, () => {
        setSpeaking(false)
        ...
        setTimeout(() => {
          ...
          startRec()  // uses proven speech recognition
        }, 600)
      }, pVoice)

    } catch(e) {
      setLivePhase('idle')
      setLiveError('Could not start. Check your connection and try again.')
      setLiveStatus('')
    }
  }
