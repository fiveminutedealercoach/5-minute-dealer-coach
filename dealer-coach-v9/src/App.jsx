    ...
    if(livePhase === 'live' && activeS) {
      ...
      try {
        const res = await fetch('/ai-proxy', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            system: 'You are ' + persona.name + ', a ' + persona.tone + ' customer at a dealership. Your ONLY concern: ' + activeS.objection.replace(/"/g,'') + '. Stay focused on that exact issue. Never introduce new objections. Never use the rep name. ' +
              'Exchange ' + newExCount + ' of 5. Escalation: ' + persona.escalation + '. ' +
              'If rep gave a strong Acknowledge + specific Value + direct Close question after exchange 2, add [CLOSE_EARNED] at end. Respond in 1-2 natural spoken sentences only.',
            messages: [{ role: 'user', content: 'Conversation: ' + fullConvo + '\n\nReact to what was just said as ' + persona.name + '. 1-2 sentences:' }],
            max_tokens: 120
        })
        const data = await res.json()
        ...
        speakReply('I hear what you are saying but I still have concerns about ' + activeS.objection.replace(/"/g,'').toLowerCase().substring(0,60) + '.')
      }
      return
    }
...
Your next reaction as ${persona.name}:`}]
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    // Fallback opener uses the ACTUAL selected script objection — always on-message
    const objectionText = script.objection.replace(/"/g, '')
    const fallbackOpener = objectionText.length > 10
      ? objectionText  // use the script objection directly as the opener
      : 'I have some concerns I need addressed before I can move forward.'
    ...
    try {
      const res = await fetch('/ai-proxy', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          system: 'You are ' + persona.name + ', a ' + persona.tone + ' customer walking into a car dealership. Your ONLY concern right now is: ' + script.objection.replace(/"/g,'') + '. State that specific objection naturally in 1-2 spoken sentences. Sound like a real person, not a script. Never use the salesperson name.',
          messages: [{ role: 'user', content: 'State your objection naturally:' }],
          max_tokens: 80
        })
      })
      const data = await res.json()
