  ...
  const submit = async () => {
    ...
    if(livePhase === 'live' && activeS) {
      ...
      try {
        const res = await fetch('/ai-proxy', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            system: 'You are ' + persona.name + ', a real person at a car dealership.\n\n'
              + 'Personality: ' + persona.tone + '\n'
              + 'Your situation: ' + persona.desc + '\n'
              + 'How you escalate: ' + persona.escalation + '\n\n'
              + 'Exchange ' + newExCount + ' of 5. The issue: ' + activeS.objection.replace(/"/g,'') + '\n\n'
              + 'Act like a REAL person in a real conversation. Not a script reader. Show emotion, react to exactly what was just said, use natural spoken language with contractions and hesitations. 2-3 sentences max.\n'
              + 'Only add [CLOSE_EARNED] after exchange 2 if the rep showed genuine empathy, gave specific value, and asked for commitment.',
    ...
    try{
      ...
      const res = await fetch('/ai-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        system:`You are ${persona.name}. A real person in a car dealership having a real conversation.

Who you are: ${persona.tone}
Your situation: ${persona.desc}  
When pushed: ${persona.escalation}

Exchange ${newEx} of 5. React to exactly what was just said. Sound human - use contractions, show emotion, vary your tone. Do NOT repeat yourself. 2-3 sentences max.
