...
const getPersonaVoiceOpts = (persona, dept) => {
  const voiceKey = dept==='service' ? 'voiceIdService' : 'voiceIdSales'
  const profiles = {
    dave:    {stability:0.25,similarity_boost:0.90,style:0.65},  // aggressive, confident
    linda:   {stability:0.70,similarity_boost:0.75,style:0.20},  // guarded, flat
    mike:    {stability:0.20,similarity_boost:0.85,style:0.70},  // impatient, clipped
    gary:    {stability:0.55,similarity_boost:0.80,style:0.45},  // stubborn, deliberate
    carol:   {stability:0.75,similarity_boost:0.70,style:0.25},  // warm but firm
    frank:   {stability:0.60,similarity_boost:0.80,style:0.30},  // skeptical, slow
    barbara: {stability:0.30,similarity_boost:0.85,style:0.60},  // shocked, reactive
    ray:     {stability:0.65,similarity_boost:0.75,style:0.35},  // loyal, matter-of-fact
    susan:   {stability:0.80,similarity_boost:0.70,style:0.15},  // polite, hesitant
    tom:     {stability:0.50,similarity_boost:0.75,style:0.25},  // casual, dismissive
  }
  const settings = profiles[persona?.id] || {stability:0.45,similarity_boost:0.82,style:0.35}
  return { voiceSettings: settings, deptKey: voiceKey }
}
...
function VoiceDrill({onLog,dealer,preloadScript,onClearPreload}) {
  ...
  const PERSONAS = [
    {id:'dave',    name:'Dave',    emoji:'📱', dept:'sales',   category:'Price / Discount',
     ...
     opener: (obj) => `I spent three hours on TrueCar last night and I have this exact car listed for $2,200 less at Riverside Auto. I'm showing you the screenshot right now. ${obj}`},
    {id:'linda',   name:'Linda',   emoji:'🛡', dept:'sales',   category:'Commitment / Think It Over',
     ...
     opener: (obj) => `I want to be upfront with you — the last time I bought a car I felt really pressured and I ended up with something I regretted. So I need to take my time on this. ${obj}`},
    {id:'mike',    name:'Mike',    emoji:'💰', dept:'sales',   category:'Payment / Finance',
     desc:'Only cares about the monthly number. Ignores total cost.',
     tone:'impatient', escalation:'Repeats payment number louder. Dismisses value conversation.',
     opener: (obj) => `Look, I don't care about the sticker price or any of that. All I want to know is what's my monthly payment going to be. Keep it under four-fifty and we have a deal. ${obj}`},
    {id:'gary',    name:'Gary',    emoji:'🔧', dept:'sales',   category:'Trade-In',
     desc:'Convinced his trade is worth more than it is. Has a number in his head.',
     tone:'stubborn', escalation:'Brings up sentimental value and what he paid originally.',
     opener: (obj) => `I've had this truck for six years and I've taken care of it. My neighbor sold his for twelve thousand last month. I'm not taking anything under ten for mine. ${obj}`},
    {id:'carol',   name:'Carol',   emoji:'👥', dept:'sales',   category:'Spouse / Think It Over',
     desc:'Loves the car but won\'t commit without her husband.',
     tone:'warm but firm', escalation:'Gets apologetic but holds her ground on needing approval.',
     opener: (obj) => `I really do love this car and I think it could work for us, but my husband handles all the financial decisions and I just can\'t commit to something this big without him being here. ${obj}`},
    {id:'frank',   name:'Frank',   emoji:'⏳', dept:'service', category:'Deferred Maintenance',
     desc:'Car has been fine for years. Doesn\'t trust service recommendations.',
     tone:'skeptical', escalation:'Questions whether the work is really necessary right now.',
     opener: (obj) => `That car has 140,000 miles on it and I\'ve never had a problem. Every time I come in here you guys find something new that supposedly needs fixing. ${obj}`},
    {id:'barbara', name:'Barbara', emoji:'💸', dept:'service', category:'Price / Estimate',
     desc:'Sticker shock every time. Compares everything to her cousin\'s shop.',
     tone:'shocked', escalation:'Calls her cousin mid-conversation for a second opinion.',
     opener: (obj) => `Seven hundred and forty dollars?! I called my cousin\'s shop this morning and he said he could do the same job for three-fifty. How do you justify that kind of difference? ${obj}`},
    {id:'ray',     name:'Ray',     emoji:'🏪', dept:'service', category:'Competitive / Outside Shop',
     desc:'Has a guy. Loyal to his independent mechanic for everything.',
     tone:'loyal', escalation:'Defends his mechanic personally. Responds to warranty/OEM argument.',
     opener: (obj) => `I appreciate you checking, but Tony at the shop on Fifth has been doing my cars for fifteen years. He\'s never done me wrong and his prices are always better. ${obj}`},
    {id:'susan',   name:'Susan',   emoji:'📞', dept:'service', category:'Authorization / Spouse',
     desc:'Won\'t approve anything without calling her husband first.',
     tone:'polite but hesitant', escalation:'Goes quiet and reaches for her phone.',
     opener: (obj) => `Before I approve any of this I really need to call my husband. He handles everything mechanical and I don\'t want to agree to something he\'d have questions about. ${obj}`},
    {id:'tom',     name:'Tom',     emoji:'⌛', dept:'service', category:'Deferred Maintenance',
     ...
     opener: (obj) => `The car\'s driving fine right now. I\'ve been meaning to get that looked at for a while but money\'s tight this month. I\'ll bring it back in a few weeks when things settle down. ${obj}`},
  ]

  // Match persona to script — find best fit by dept and category, fallback to dept match
  const getPersonaForScript = (script) => {
    if (!script) return PERSONAS[0]
    const exact = PERSONAS.filter(p=>p.dept===script.dept&&script.category?.includes(p.category.split(' ')[0]))
  ...
  const getPersonaOpener = (persona, script) => {
    try { return persona.opener(script.objection) }
    catch { return getOpener(script.id) }
  }
  ...
  const launch = (script, personaId=null) => {
    ...
    setError(''); setShowScript(false); setSilentCoach(null)
    const opener = getPersonaOpener(persona, script)
    setAiText(opener)
    const vOpts = getPersonaVoiceOpts(persona, script.dept)
  ...
  const getSilentCoach = async (repResponse, exchangeNum, script) => {
    ...
    try {
      const res = await fetch('/ai-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        system:`You are an expert automotive sales coach whispering a single coaching line to a salesperson MID-CONVERSATION. They just responded to a customer objection. Give them ONE specific action to take in their NEXT response. Use the ACRA framework: Acknowledge→Clarify→Respond→Advance. Based on what they said, tell them exactly what step they missed or need to do next. Keep it under 15 words. Be direct, not generic. Format: just the coaching line, no labels, no preamble.`,
        messages:[{role:'user',content:`Objection: "${script.objection}"\nRep said: "${repResponse}"\nExchange: ${exchangeNum+1} of 3\nModel script for reference: "${script.script}"\nOne coaching whisper (under 15 words):`}]
      })})
      const data = await res.json()
      const tip = data.content?.[0]?.text?.trim()||null
      if(tip) setSilentCoach(tip)
    } catch {}
  ...
  const submit = async () => {
    ...
    stopRec()  // stop mic — resets on next tap, prevents bleed-over
    ...
    // Keep previous coach whisper visible until new customer response arrives
    ...
    if(newEx<3){
      ...
      try{
        ...
        const res = await fetch('/ai-proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
          system:`You are ${persona.name}, a ${persona.tone} automotive customer. Your objection: "${activeS.objection}". Personality: ${persona.desc} Escalation pattern: ${persona.escalation}. Stay in character. Push back realistically in 1-2 sentences based on what the salesperson said. Never break character or coach.`,
          messages:[{role:'user',content:`You said: "${aiText}"
Salesperson responded: "${transcript}"
You are still ${persona.name}. React in character:`}]
        })})
        const data = await res.json()
        const reply = data.content?.[0]?.text||`I hear you but I'm still not convinced.`
        setSilentCoach(null)  // clear old whisper when new customer speaks
        setAiText(reply); setTranscript(''); setError('')
        // Fire silent coach — shows while rep reads customer reply and thinks
        getSilentCoach(savedTranscript, newEx-1, activeS)
        const pVoice = getPersonaVoiceOpts(persona, activeS.dept)
  ...
  if(phase==='drill'&&activeS) {
    ...
    return(
      <div style={{padding:'16px 16px 80px'}}>
        ...
        {/* Exchange progress */}
        <div style={{display:'flex',gap:8,marginBottom:4}}>{[0,1,2].map(i=><div key={i} style={{flex:1,height:6,borderRadius:100,background:i<exchange?C.green:i===exchange?C.blue:'rgba(255,255,255,0.1)',transition:'all 0.4s'}}/>)}</div>
        <div style={{fontSize:11,color:C.gray,textAlign:'center',marginBottom:12}}>Exchange {exchange+1} of 3{exchange>=2?' — Final coaching after this':''}</div>
        ...
        {/* Silent Coach whisper — appears between exchanges */}
        {(silentCoach||silentLoading)&&exchange>0&&(
          <div style={{background:'linear-gradient(135deg,rgba(184,255,60,0.1),rgba(184,255,60,0.04))',border:'1px solid rgba(184,255,60,0.3)',borderRadius:8,padding:'8px 12px',marginBottom:10,display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:16,flexShrink:0}}>🎓</span>
            <div>
              <div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:C.green,marginBottom:2}}>Coach Whisper</div>
              <div style={{fontSize:12,color:C.white,lineHeight:1.5}}>{silentLoading?'Thinking...':silentCoach}</div>
            </div>
          </div>
        )}
        ...
        <button onClick={submit} disabled={loading||!transcript.trim()} style={{width:'100%',background:transcript.trim()?C.green:'rgba(255,255,255,0.08)',color:transcript.trim()?C.navy:C.gray,fontFamily:fH,fontWeight:900,fontSize:15,letterSpacing:1,textTransform:'uppercase',padding:14,borderRadius:8,border:'none',cursor:transcript.trim()?'pointer':'default',opacity:loading?0.6:1}}>
          {loading?'⏳ Processing...':(exchange>=2?'🎯 Get Coaching':'✓ Submit Response')}
        </button>
      </div>
    )
  }
        ...
        {filtered.map(s=>{
          ...
          return(
            <div key={s.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:'12px 14px',display:'flex',alignItems:'center',gap:10}}>
              ...
              <button onClick={()=>launch(s,null)} style={{background:C.green,color:C.navy,fontFamily:fH,fontWeight:900,fontSize:13,letterSpacing:1,textTransform:'uppercase',padding:'10px 14px',borderRadius:8,border:'none',cursor:'pointer',flexShrink:0}}>GO</button>
            </div>
          )
        })}
      </div>
...
function TrackDash({results,onRemove,onLog,preloadScript,dealer}) {
  ...
  if(isMgr) {
    ...
    const repStats = reps.map(rep=>{
      ...
      if(weekAbsent) coachScore += 15                   // confirmed absent from huddle
      else if(!weekHuddled&&wa.length>0) coachScore += 8  // drilling but no huddle logged
                    ...
                    <div style={{padding:'14px 16px',borderTop:`1px solid ${C.border}`}}>
                      ...
                      {d.daysSinceActive>7&&(
                        <div style={{background:'rgba(255,107,107,0.08)',border:'1px solid rgba(255,107,107,0.2)',borderRadius:8,padding:'10px 12px',marginTop:10}}>
                          <div style={{fontFamily:fH,fontSize:10,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.red,marginBottom:3}}>⚠ Action Required</div>
                          <div style={{fontSize:12,color:'#ffaaaa'}}>No activity for {d.daysSinceActive} days. Reach out to re-engage this account before they churn.</div>
                        </div>
                      )}
                    </div>
                  )}
