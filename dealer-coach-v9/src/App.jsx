...
Your next reaction as ${persona.name}:`}]
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    try {
      ...
      // Mic track reference  -  mute while AI speaks so it never hears itself
      const micTrack = micStream.getTracks()[0]
      let aiSpeaking = false
      const muteMic   = () => { if (micTrack) micTrack.enabled = false }
      const unmuteMic = () => { if (micTrack) micTrack.enabled = true }
      const sendEvent = (obj) => { if (dc && dc.readyState === "open") dc.send(JSON.stringify(obj)) }

      // Mute mic immediately  -  stays muted until AI finishes first response
      muteMic()
      ...
      dc.onopen = () => {
        ...
        // Update voice + transcription only  -  do NOT override persona instructions
        ...
        setTimeout(() => sendEvent({ type: "response.create" }), 600)
      }
      ...
      dc.onmessage = e => {
        try {
          ...
          // ── MIC MUTE MANAGEMENT ──────────────────────────────
          // Mute mic while AI speaks  -  foolproof echo prevention

          if (evt.type === 'response.created' ||
              evt.type === 'output_audio_buffer.started') {
            aiSpeaking = true
            muteMic()
          }

          // Audio fully done playing  -  unmute mic
          if (evt.type === 'output_audio_buffer.stopped') {
            setTimeout(() => {
              aiSpeaking = false
              unmuteMic()
            }, 800) // 800ms after audio stops before listening
          }

          // Fallback  -  response.done fires when generation ends
          // Add generous delay since audio plays after generation
