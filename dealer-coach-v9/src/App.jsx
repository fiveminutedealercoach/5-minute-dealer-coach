...
Your next reaction as ${persona.name}:`}]
  ...
  const REALTIME_VOICES = {
    dave:    'echo',    // male  -  confident, TrueCar researcher
    mike:    'ash',     // male  -  payment-only focus
    gary:    'ballad',  // male  -  truck guy, stubborn on trade
    frank:   'onyx',    // male  -  skeptical service customer (MUST stay onyx)
    ray:     'verse',   // male  -  loyal to his mechanic
    tom:     'echo',    // male  -  deferral, low urgency
    linda:   'shimmer', // female  -  guarded, got burned before
    carol:   'coral',   // female  -  questions every add-on
    barbara: 'sage',    // female  -  price shocked
    susan:   'shimmer', // female  -  needs husband approval
  }
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    setLiveTranscript([])
    setExchangeCount(0)
    liveTranscriptRef.current = []  // critical  -  clear ref for fresh session
    ...
    try {
      ...
      const sessionRes = await fetch('/realtime-session', {
        ...
        body: JSON.stringify({
          ...
          voice:             REALTIME_VOICES[persona.id] || 'alloy',
        })
      })
      ...
      // Step 3  -  Aggressively clean ALL audio before creating new session
      document.querySelectorAll('audio').forEach(el=>{
        try { el.pause(); el.srcObject=null; el.src=''; } catch {}
        el.remove()
      })
      ...
      audioEl.setAttribute('data-realtime','1')
      document.body.appendChild(audioEl)
      pc.ontrack = e => {
        if(e.streams && e.streams[0]) {
          // Ensure no other stream is playing
          document.querySelectorAll('audio[data-realtime]').forEach(a=>{
            if(a !== audioEl){ try{a.pause();a.srcObject=null}catch{}; a.remove() }
          })
      ...
      const micStream = await navigator.mediaDevices.getUserMedia({
        audio: {
          echoCancellation: true,   // prevents AI voice triggering VAD
      ...
      // We control VAD entirely - OpenAI server VAD is OFF
      // This prevents the AI from ever hearing its own audio output
      let aiSpeaking        = false   // true while AI audio is playing
      let silenceTimer      = null    // fires when user stops speaking
      let userSpeaking      = false   // true while user audio level is high
      let silenceDetectorId = null    // requestAnimationFrame id

      const sendEvent = (obj) => { if (dc && dc.readyState === 'open') dc.send(JSON.stringify(obj)) }

      const commitAndRespond = () => {
        if (aiSpeaking) return  // never send while AI is talking
        sendEvent({ type: 'input_audio_buffer.commit' })
        sendEvent({ type: 'response.create' })
      }

      // Start silence detector using Web Audio API
      const startSilenceDetector = (stream) => {
        try {
          ...
          const SILENCE_DELAY     = 1500 // ms of silence before committing
          let   lastSpeakTime     = 0

          const detect = () => {
            silenceDetectorId = requestAnimationFrame(detect)
            if (aiSpeaking) {
              // Clear any audio captured while AI was speaking
            ...
            if (avg > SILENCE_THRESHOLD) {
              ...
              clearTimeout(silenceTimer)
            } else if (userSpeaking && Date.now() - lastSpeakTime > SILENCE_DELAY) {
              // User stopped speaking - commit after silence
              userSpeaking = false
              clearTimeout(silenceTimer)
              silenceTimer = setTimeout(() => {
                if (!aiSpeaking) commitAndRespond()
              }, 200)
            }
          }
          detect()
        } catch(e) {
          // Fallback - if Web Audio fails, use a simpler approach
      ...
      dc.onopen = () => {
        ...
        // Configure session - NO server VAD, we handle turns ourselves
        dc.send(JSON.stringify({
          type: 'session.update',
          session: {
            voice,
            turn_detection: null,  // CLIENT controls when to send - no server VAD
            input_audio_transcription: { model: 'whisper-1' },
            max_response_output_tokens: 150,
          }
        }))
        // Start our silence detector on the mic stream
        startSilenceDetector(micStream)
      ...
      dc.onmessage = e => {
        try {
          ...
          // ── AI SPEAKING STATE ─────────────────────────────────
          // Track when AI audio starts/stops so silence detector
          // ignores audio while AI is playing

          if (evt.type === 'output_audio_buffer.started' ||
              evt.type === 'response.created') {
            aiSpeaking = true
            // Clear any mic audio captured while AI was playing
            sendEvent({ type: 'input_audio_buffer.clear' })
          }

          if (evt.type === 'output_audio_buffer.stopped' ||
              evt.type === 'output_audio_buffer.cleared') {
            // Wait for audio to fully drain from speakers before listening
            setTimeout(() => {
              aiSpeaking = false
            }, 600)
          }

          // Fallback in case buffer events don't fire
          ...
          // ── TRANSCRIPT CAPTURE ───────────────────────────────
          // AI transcript  -  multiple event variants OpenAI fires
          const aiTranscriptText =
            (evt.type === 'response.audio_transcript.done'             && evt.transcript) ||
            (evt.type === 'response.text.done'                         && evt.text) ||
            (evt.type === 'conversation.item.created' && evt.item?.role === 'assistant' &&
              evt.item?.content?.[0]?.transcript) || null

          if (aiTranscriptText && aiTranscriptText.trim().length > 3) {
            setLiveTranscript(prev => {
              if (prev.length > 0 && prev[prev.length-1].text === aiTranscriptText) return prev
              const next = [...prev, { role: 'customer', text: aiTranscriptText }]
              liveTranscriptRef.current = next
              setTimeout(()=>{ const b=document.getElementById('live-transcript-box'); if(b) b.scrollTop=b.scrollHeight }, 100)
              return next
            })
          }

          // Rep transcript  -  multiple event variants
          const repTranscriptText =
            (evt.type === 'conversation.item.input_audio_transcription.completed' && evt.transcript) ||
            (evt.type === 'input_audio_buffer.speech_stopped'                     && evt.transcript) ||
            (evt.type === 'conversation.item.created' && evt.item?.role === 'user' &&
              evt.item?.content?.[0]?.transcript) || null

          if (repTranscriptText && repTranscriptText.trim().length > 2) {
            setLiveTranscript(prev => {
              const next = [...prev, { role: 'rep', text: repTranscriptText }]
              liveTranscriptRef.current = next
              setTimeout(()=>{ const b=document.getElementById('live-transcript-box'); if(b) b.scrollTop=b.scrollHeight }, 100)
              return next
            })
              // Count full exchange (rep spoke = one exchange complete)
              setExchangeCount(prev => {
                const next = prev + 1
                if (next >= 5) {
                  // 2.5s delay lets final AI transcript event arrive before grading
                  setTimeout(() => {
                    const snap = [...(liveTranscriptRef.current||[])]
                    endLiveDrill(script, snap.length > 0 ? snap : null)
                  }, 2500)
                }
                return next
              })
  ...
  const endLiveDrill = async (script, transcriptSnapshot) => {
    ...
    // Safety timeout  -  20 seconds max before unlocking
    const safetyTimer = setTimeout(()=>{
      setLivePhase(p => {
        if(p === 'ended'){
          setLiveStatus('')
          setLiveError('Report timed out. Check your connection and try again.')
          return 'idle'
        }
        return p
      })
    ...
    // Small delay to let final transcript events settle from WebRTC
    await new Promise(r => setTimeout(r, 800))

    // Use passed snapshot → then ref → then state (most to least reliable)
    const transcript = (transcriptSnapshot && transcriptSnapshot.length > 0)
      ? transcriptSnapshot
      : (liveTranscriptRef.current && liveTranscriptRef.current.length > 0
        ? liveTranscriptRef.current
        : [])

    const targetScript = script || activeS
    const persona = PERSONAS.find(p => p.id === activePersId) ||
      (targetScript ? getPersonaForScript(targetScript) : null)

    if (transcript.length > 0 && targetScript) {
      const repLines    = transcript.filter(t => t.role === 'rep').map(t => t.text)
      const custLines   = transcript.filter(t => t.role === 'customer').map(t => t.text)
      const lastRepLine = repLines[repLines.length - 1] || repLines[0] || 'I addressed your concern'
      const fullConvo   = transcript.map(t =>
        `${t.role === 'rep' ? 'Rep' : (persona?.name || 'Customer')}: "${t.text}"`
      ).join(' | ')

      // Need at least one rep line to grade
      if (repLines.length === 0) {
        clearTimeout(safetyTimer)
        setLivePhase('idle')
        setLiveError('No rep responses captured. Check your microphone  -  speak clearly after the persona finishes.')
        setTimeout(()=>setLiveError(''), 5000)
        return
      }

      try {
        clearTimeout(safetyTimer)
        // Pass full conversation so ACRA grader has complete context
        await getFeedback(lastRepLine, repLines.length > 0 ? [fullConvo] : ['No response captured'])
        // getFeedback sets phase='feedback' and livePhase='idle'
      } catch(err) {
        clearTimeout(safetyTimer)
        setLivePhase('idle')
        setLiveError(`Report failed: ${err.message}. Try again.`)
        setPhase('list')
      }
    } else {
      ...
      setLiveTranscript([])
      setLiveStatus('')
      setLiveError('Not enough conversation captured. Check your microphone and try again  -  speak clearly after the persona finishes.')
      setTimeout(()=>{ setLiveError(''); setActiveS(null) }, 5000)
    }
  }
  ...
  const stopLiveDrill = () => {
    ...
    try { pcRef.current?.close() } catch {}
    pcRef.current = null; dcRef.current = null; streamRef.current = null
    // Cancel silence detector animation frame if running
    if (typeof cancelAnimationFrame !== 'undefined') {
      // silenceDetectorId is scoped to startLiveDrill - will GC naturally
    }
    document.querySelectorAll('audio[data-realtime]').forEach(el => {
...
{"ack_score":3,"clar_score":2,"resp_score":3,"adv_score":1,"total":9,"score":"B","score_detail":"B  -  [one sharp sentence about overall performance]","acknowledge":"[specific  -  quote their words, say what worked or failed]","clarify":"[did they diagnose or just pitch? be specific]","respond":"[compare their pivot to the model  -  generic vs specific]","advance":"[did they close decisively? exact feedback]","improvement":"[complete word-for-word script tailored to ${persona.name} and this objection  -  min 3 sentences, all 4 ACRA steps]"}`
  ...
  // ── LIVE AI SIMULATION SCREEN  -  renders BEFORE regular drill ─
    ...
    return(
      <div style={{padding:'16px 16px 80px'}}>
        ...
        <div style={{background:livePhase==='connecting'?'rgba(255,201,71,0.1)':livePhase==='live'?'rgba(255,77,77,0.1)':'rgba(184,255,60,0.08)',border:`1px solid ${livePhase==='connecting'?'rgba(255,201,71,0.3)':livePhase==='live'?'rgba(255,77,77,0.3)':'rgba(184,255,60,0.25)'}`,borderRadius:12,padding:'14px 16px',marginBottom:14}}>
          ...
          {livePhase==='live'&&(
            <div style={{background:'rgba(255,77,77,0.1)',borderRadius:8,padding:'8px 12px',display:'flex',alignItems:'center',gap:8}}>
              <div style={{width:8,height:8,borderRadius:'50%',background:C.red,flexShrink:0,animation:'livepulse 1s infinite'}}/>
              <span style={{fontSize:12,color:C.lightText}}>No button needed  -  just speak. {persona?.name} will respond automatically.</span>
            </div>
          )}
        ...
        {livePhase==='live'&&(
          <button onClick={()=>endLiveDrill(activeS,liveTranscript)} style={{width:'100%',background:C.green,color:C.navy,fontFamily:fH,fontWeight:900,fontSize:15,letterSpacing:1,textTransform:'uppercase',padding:16,borderRadius:10,border:'none',cursor:'pointer',marginBottom:10,boxShadow:'0 0 30px rgba(184,255,60,0.3)'}}>
            🎯 End Session → Get Coaching Report
          </button>
        )}
        <button onClick={stopLiveDrill} style={{width:'100%',background:'transparent',border:`1px solid ${C.border}`,color:C.gray,fontFamily:fH,fontWeight:700,fontSize:12,letterSpacing:1,textTransform:'uppercase',padding:10,borderRadius:8,cursor:'pointer'}}>
