  ...
  const startLiveDrill = async (script, persona) => {
    ...
    try {
      ...
      dc.onopen = () => {
        ...
        console.log('[LiveAI] Connected  -  persona:', persona.id, 'voice:', voice)
        // Send session update with correct persona voice and VAD settings
        dc.send(JSON.stringify({
          ...
          session: {
            ...
            turn_detection: {
              type: 'server_vad',
              threshold: 0.90,           // high  -  only clear intentional speech
              prefix_padding_ms: 500,
              silence_duration_ms: 2000, // 2s silence before AI responds  -  prevents cut-off
              create_response: true,
            },
            input_audio_transcription: { model: 'whisper-1' },
            max_response_output_tokens: 100,  // short responses = less cross-talk window
          }
        }))
        // Wait for session update to apply before triggering first response
        setTimeout(() => {
          dc.send(JSON.stringify({ type: 'response.create' }))
        }, 800)
      }

      // VAD helpers  -  scoped to this session
      let aiAudioPlaying = false
      const sendEvent = (obj) => { if (dc && dc.readyState === 'open') dc.send(JSON.stringify(obj)) }
      const enableVAD  = () => sendEvent({ type:'session.update', session:{ turn_detection:{ type:'server_vad', threshold:0.90, prefix_padding_ms:500, silence_duration_ms:2000, create_response:true } } })
      const disableVAD = () => { sendEvent({ type:'session.update', session:{ turn_detection:null } }); sendEvent({ type:'input_audio_buffer.clear' }) }
      ...
      dc.onmessage = e => {
        try {
          ...
          // ── AI TURN MANAGEMENT ────────────────────────────────
          // AI is about to speak  -  kill VAD immediately + clear buffer
          if (evt.type === 'response.created' || evt.type === 'response.output_item.added') {
            aiAudioPlaying = true
            disableVAD()
          }

          // Audio buffer started playing  -  VAD must stay off
          if (evt.type === 'output_audio_buffer.started') {
            aiAudioPlaying = true
            disableVAD()
          }

          // Audio buffer fully drained  -  NOW safe to re-enable VAD
          if (evt.type === 'output_audio_buffer.stopped' || evt.type === 'output_audio_buffer.cleared') {
            aiAudioPlaying = false
            // Wait 800ms after audio truly stops before listening again
            setTimeout(() => {
              if (!aiAudioPlaying) {
                enableVAD()
          ...
          // Fallback: response.done + generous delay for audio to finish playing
          if (evt.type === 'response.done') {
            setTimeout(() => {
              if (!aiAudioPlaying) enableVAD()
            }, 3000) // 3s is enough for even long responses to finish playing
          }
