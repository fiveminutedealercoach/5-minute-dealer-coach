...
function VoiceDrill({onLog,dealer,preloadScript,onClearPreload}) {
  ...
  const submit = async () => {
    ...
    if(newEx<3){
      ...
      try{
        // Get silent coaching whisper while customer is thinking
        getSilentCoach(transcript, newEx-1, activeS)
  ...
  if(phase==='drill'&&activeS) {
    ...
    return(
      <div style={{padding:'16px 16px 80px'}}>
        ...
        {(silentCoach||silentLoading)&&exchange>0&&exchange<3&&(
...
function TrackDash({results,onRemove,onLog,preloadScript,dealer}) {
  ...
  if(isMgr) {
    ...
    const repStats = reps.map(rep=>{
      ...
      const weekHuddled = wa.some(a=>a.type==='huddle')
      ...
      if(!weekHuddled&&wa.length>0) coachScore += 10    // drilling but skipping huddles
      ...
      return{
        ...
        coachReasons:[
          ...
          !weekHuddled&&wa.length>0?'Missing huddles':null,
        ].filter(Boolean)
      }
