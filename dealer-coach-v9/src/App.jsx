...
function VoiceDrill({onLog,dealer,preloadScript,onClearPreload}) {
  ...
  const PERSONAS = [
    // SALES
    {id:'dave',    name:'Dave',    emoji:'📱', dept:'sales',
     cats:['Handling Objections & Value Selling'],
     ...
     opener:(obj)=>`I spent three hours on TrueCar last night and I have this exact car listed for two thousand two hundred less at Riverside Auto. I have the screenshot right here. ${obj}`},
    {id:'linda',   name:'Linda',   emoji:'🛡', dept:'sales',
     cats:['Mindset & Gross Awareness','Handling Objections & Value Selling'],
    ...
    {id:'mike',    name:'Mike',    emoji:'💰', dept:'sales',
     cats:['Menu Selling & Finance'],
     ...
     opener:(obj)=>`Look, I don't care about the sticker price or any of that. All I want to know is my monthly payment. Keep it under four-fifty and we have a deal. ${obj}`},
    {id:'gary',    name:'Gary',    emoji:'🚛', dept:'sales',
     cats:['Used Car Gross & Reconditioning'],
     ...
     opener:(obj)=>`I've had this truck for six years and taken immaculate care of it. My neighbor sold his last month for twelve thousand. I'm not taking anything under ten for mine. ${obj}`},
    {id:'carol',   name:'Carol',   emoji:'👥', dept:'sales',
     cats:['Add-Ons & After-Sale','Sales Tactics for Higher Gross'],
     desc:'Questions every add-on. Thinks she can get it cheaper online.',
     tone:'warm but skeptical', escalation:'Compares each item to online prices.',
     opener:(obj)=>`I really like the car but I think a lot of these extras are things I could add later or honestly get cheaper somewhere else. ${obj}`},
    // SERVICE
    {id:'frank',   name:'Frank',   emoji:'⏳', dept:'service',
     cats:['MPI Conversion','Selling the Menu & Recommended Services'],
     desc:'Car has been fine for years. Skeptical of every recommendation.',
     tone:'skeptical', escalation:'Questions whether any work is really necessary right now.',
     opener:(obj)=>`That car has a hundred and forty thousand miles on it and I've never had a problem. Every time I come in here you guys find something new that supposedly needs fixing. ${obj}`},
    {id:'barbara', name:'Barbara', emoji:'💸', dept:'service',
     cats:['Handling Objections & Price Pushback'],
     desc:'Sticker shock every time. Compares to her cousin's shop.',
     tone:'shocked', escalation:'Mentions her cousin's shop. Gets more agitated if not explained.',
    ...
    {id:'ray',     name:'Ray',     emoji:'🏪', dept:'service',
     cats:['Handling Objections & Price Pushback','Selling Specific Services'],
     desc:'Has a guy. Loyal to his independent mechanic for fifteen years.',
     tone:'loyal', escalation:'Defends his mechanic personally. Responds to warranty/OEM argument.',
     opener:(obj)=>`I appreciate you checking, but my guy Tony at the shop on Fifth has been doing my cars for fifteen years. He's never done me wrong and his prices are always better. ${obj}`},
    {id:'susan',   name:'Susan',   emoji:'📞', dept:'service',
     cats:['MPI Conversion','Phone Ups & Appointment Setting'],
    ...
    {id:'tom',     name:'Tom',     emoji:'⌛', dept:'service',
     cats:['Mindset & Customer-Pay Focus','Selling the Menu & Recommended Services'],
     ...
     opener:(obj)=>`The car's driving fine right now. Money's tight this month. I'll bring it back in a few weeks when things settle down. ${obj}`},
  ]

  // Match persona using actual script categories
  const getPersonaForScript = (script) => {
    if(!script) return PERSONAS[0]
    const exact = PERSONAS.filter(p=>p.dept===script.dept&&p.cats.includes(script.category))
