...
Your next reaction as ${persona.name}:`}]
    ...
    try {
      ...
      dc.onmessage = e => {
        try {
          ...
          // AI starts responding — disable VAD to prevent cross-talk
          if (evt.type === 'response.created') {
            if (dcRef.current && dcRef.current.readyState === 'open') {
              dcRef.current.send(JSON.stringify({
                type: 'session.update',
                session: { turn_detection: null }  // disable VAD while AI speaks
              }))
            }
          }

          // AI finished speaking — re-enable VAD
          if (evt.type === 'response.done') {
            ...
            }, 500) // small delay after AI done before re-enabling
          }

          // AI (persona) transcript — fires when AI finishes speaking
          if (evt.type === 'response.audio_transcript.done') {
            if (evt.transcript && evt.transcript.trim().length > 3) {
              setLiveTranscript(prev => {
                if (prev.length > 0 && prev[prev.length-1].text === evt.transcript) return prev
                const next = [...prev, { role: 'customer', text: evt.transcript }]
          ...
          if (evt.type === 'conversation.item.input_audio_transcription.completed') {
            if (evt.transcript && evt.transcript.trim().length > 2) {
              setLiveTranscript(prev => {
                const next = [...prev, { role: 'rep', text: evt.transcript }]
                liveTranscriptRef.current = next
                setTimeout(()=>{ const b=document.getElementById('live-transcript-box'); if(b) b.scrollTop=b.scrollHeight }, 100)
                return next
              })
  ...
  const endLiveDrill = async (script, transcriptSnapshot) => {
    ...
    if (transcript.length > 0 && targetScript) {
      ...
      const lastRepLine = repLines[repLines.length - 1] || 'I tried to handle your concern'
      ...
      try {
        ...
        await getFeedback(lastRepLine, [fullConvo])
        // getFeedback internally calls setPhase('feedback') and setLivePhase('idle')
      } catch(err) {
        clearTimeout(safetyTimer)
        setLivePhase('idle')
        setLiveError('Could not generate report — please try again.')
        setPhase('list')
      }
    } else {
