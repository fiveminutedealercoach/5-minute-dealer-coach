...
Your next reaction as ${persona.name}:`}]
  ...
  const startLiveDrill = async (script, persona) => {
    ...
    try {
      ...
      // Clean up any existing audio elements first — prevents crosstalk
      document.querySelectorAll('audio[data-realtime]').forEach(el => el.remove())
          ...
          if (evt.type === 'conversation.item.input_audio_transcription.completed') {
            if (evt.transcript && evt.transcript.trim().length > 2) {
              ...
              setExchangeCount(prev => {
                const next = prev + 1
                if (next >= 5) {
                  setTimeout(() => endLiveDrill(script, liveTranscriptRef.current), 2000)
                }
                return next
              })
            }
  ...
  const endLiveDrill = async (script, transcriptSnapshot) => {
    ...
    // Fallback — if stuck for 15 seconds, reset so user isn't locked out
    setTimeout(()=>{
      setLivePhase(p => {
        if(p === 'ended'){ setLiveStatus(''); setLiveError('Report timed out — try again.'); return 'idle' }
        return p
      })
    }, 15000)
    ...
    if (transcript.length > 0 && script) {
      ...
      const lastRepLine = repLines[repLines.length - 1] || ''
      const fullConvo   = transcript.map(t =>
        `${t.role === 'rep' ? 'Rep' : (persona?.name || 'Customer')}: "${t.text}"`
      ).join(' | ')
      // Pass full conversation as the allResps array so grading has full context
