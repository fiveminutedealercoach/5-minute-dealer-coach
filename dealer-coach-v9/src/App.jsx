    ...
    try {
      ...
      dc.onopen = () => {
        ...
        dc.send(JSON.stringify({
          ...
          session: {
            voice,
            turn_detection: { type: 'server_vad', threshold: 0.5, prefix_padding_ms: 500, silence_duration_ms: 1200 },
          ...
          if (evt.type === 'response.audio_transcript.done') {
            if (evt.transcript) {
              ...
              setExchangeCount(prev => {
                const next = prev + 1
                if (next >= 5) setTimeout(() => endLiveDrill(script, liveTranscriptRef.current), 1500)
                return next
              })
            }
          }
