  ...
  return (
    <div style={{paddingBottom:80}}>
      ...
      <div style={{padding:'0 16px'}}>
        ...
        {/* Live Drills Today — prominent counter */}
        {isMgr ? (
          <div style={{background:'linear-gradient(135deg,rgba(184,255,60,0.1),rgba(184,255,60,0.04))',border:'1px solid rgba(184,255,60,0.3)',borderRadius:12,padding:'14px 16px',marginBottom:12,display:'flex',alignItems:'center',gap:14}}>
            <div style={{background:'rgba(184,255,60,0.15)',borderRadius:10,padding:'10px 14px',textAlign:'center',minWidth:64}}>
              <div style={{fontFamily:fH,fontSize:38,fontWeight:900,color:C.green,lineHeight:1}}>{teamKPI?.totalDrills??'—'}</div>
              <div style={{fontFamily:fH,fontSize:8,fontWeight:700,letterSpacing:1.5,textTransform:'uppercase',color:C.green,marginTop:2}}>Total</div>
            </div>
            <div style={{flex:1}}>
              <div style={{fontFamily:fH,fontSize:13,fontWeight:900,textTransform:'uppercase',color:C.white,marginBottom:2}}>Team Drills</div>
              <div style={{fontSize:12,color:C.gray,marginBottom:6}}>Across all reps · all time</div>
              <div style={{display:'flex',gap:10}}>
                <div style={{fontSize:11,color:C.blue}}>🏆 {teamKPI?.salesDrills??0} Sales</div>
                <div style={{fontSize:11,color:C.green}}>🔧 {teamKPI?.svcDrills??0} Service</div>
              </div>
            </div>
            <div style={{textAlign:'right'}}>
              <div style={{fontFamily:fH,fontSize:11,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:teamKPI?.winRate>=60?C.green:teamKPI?.winRate>=40?C.yellow:C.orange}}>{teamKPI?`${teamKPI.winRate}% Win Rate`:'—'}</div>
            </div>
          </div>
        ) : (
        ...
        {/* Stats row — managers see team KPIs, reps see personal */}
        {isMgr ? (
          <div style={{marginBottom:12}}>
            <div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:C.green,marginBottom:8}}>📊 Dealership Totals</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
              {[
                {val:teamKPI?.activeReps??'—',label:'Active Reps',color:C.blue},
                {val:teamKPI?.totalDrills??'—',label:'Team Drills',color:C.green},
                {val:teamKPI?.weekHuddles??'—',label:'Huddles / Week',color:C.yellow},
                {val:teamKPI?`${teamKPI.winRate}%`:'—',label:'Win Rate',color:teamKPI?.winRate>=60?C.green:teamKPI?.winRate>=40?C.yellow:C.orange},
              ].map(({val,label,color})=>(
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              <div style={{background:'rgba(26,107,255,0.08)',border:'1px solid rgba(26,107,255,0.2)',borderRadius:10,padding:'8px 12px',display:'flex',alignItems:'center',gap:10}}>
                <span style={{fontSize:16}}>🏆</span>
                <div><div style={{fontFamily:fH,fontSize:18,fontWeight:900,color:C.blue}}>{teamKPI?.salesDrills??'—'}</div><div style={{fontFamily:fH,fontSize:8,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.blue}}>Sales Drills</div></div>
              </div>
              <div style={{background:'rgba(184,255,60,0.07)',border:'1px solid rgba(184,255,60,0.2)',borderRadius:10,padding:'8px 12px',display:'flex',alignItems:'center',gap:10}}>
                <span style={{fontSize:16}}>🔧</span>
                <div><div style={{fontFamily:fH,fontSize:18,fontWeight:900,color:C.green}}>{teamKPI?.svcDrills??'—'}</div><div style={{fontFamily:fH,fontSize:8,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.green}}>Service Drills</div></div>
              </div>
            </div>
          </div>
        ) : (
