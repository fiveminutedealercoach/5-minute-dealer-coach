...
export async function onRequestPost(context) {
  ...
  try {
    const { personaName, personaDesc, personaTone, personaEscalation, objection, dept, scriptText } = await request.json()
...
- Sound like a real ${personaTone} person having a real conversation`
    ...
    const response = await fetch('https://api.openai.com/v1/realtime/sessions', {
      ...
      body: JSON.stringify({
        ...
        voice: 'alloy', // Will be overridden per persona on frontend
        instructions,
