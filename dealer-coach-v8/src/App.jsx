...
// ══════════════════════════════════════════════════════════════
// TRACK & DASHBOARD — Dept indicators, simplified GM tabs
// ══════════════════════════════════════════════════════════════
function TrackDash({results,onRemove,onLog,preloadScript,dealer}) {
  ...
  useEffect(()=>{if(preloadScript)setForm(f=>({...f,script:preloadScript}))},[preloadScript])
  useEffect(()=>{
  ...
  // GM: Team View | My Activity | Manual Log
  // Rep: My Drills | Log Result
  const tabs = isMgr
    ? [{id:'dashboard',label:'👥 Team View'},{id:'my',label:'My Activity'},{id:'log',label:'Manual Log'}]
  ...
  return(
    <div style={{padding:'16px 16px 80px'}}>
      ...
      <div style={{display:'flex',gap:2,marginBottom:16,borderBottom:`1px solid ${C.border}`}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{background:'transparent',border:'none',borderBottom:activeTab===t.id?`2px solid ${C.green}`:'2px solid transparent',color:activeTab===t.id?C.white:C.gray,fontFamily:fH,fontSize:11,fontWeight:activeTab===t.id?900:600,letterSpacing:1,textTransform:'uppercase',padding:'8px 10px',cursor:'pointer',marginBottom:-1,flexShrink:0}}>{t.label}</button>
        ))}
      </div>
      ...
      {activeTab==='dashboard'&&(
        <div>
          ...
          {!dashLoading&&dashData&&!dashData.error&&(()=>{
            const acts     = dashData.activities||[]
            const reps     = [...new Set(acts.map(a=>a.repName))].filter(Boolean)
            const salesActs = acts.filter(a=>a.dept==='sales')
            const svcActs   = acts.filter(a=>a.dept==='service')
            const repStats  = reps.map(rep=>{
              const ra=acts.filter(a=>a.repName===rep)
              return{rep,total:ra.length,won:ra.filter(a=>a.result==='won').length,progress:ra.filter(a=>a.result==='progress').length,practice:ra.filter(a=>a.result==='practice').length,sales:ra.filter(a=>a.dept==='sales').length,service:ra.filter(a=>a.dept==='service').length}
            ...
            return <>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:14}}>
                <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:'12px 14px',textAlign:'center'}}><div style={{fontFamily:fH,fontSize:30,fontWeight:900,color:C.blue}}>{reps.length}</div><div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.blue,marginTop:3}}>Active Reps</div></div>
                <div style={{background:'rgba(26,107,255,0.08)',border:'1px solid rgba(26,107,255,0.25)',borderRadius:10,padding:'12px 14px',textAlign:'center'}}><div style={{fontFamily:fH,fontSize:30,fontWeight:900,color:C.blue}}>{salesActs.length}</div><div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.blue,marginTop:3}}>🏆 Sales</div></div>
                <div style={{background:'rgba(184,255,60,0.07)',border:'1px solid rgba(184,255,60,0.25)',borderRadius:10,padding:'12px 14px',textAlign:'center'}}><div style={{fontFamily:fH,fontSize:30,fontWeight:900,color:C.green}}>{svcActs.length}</div><div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:C.green,marginTop:3}}>🔧 Service</div></div>
              </div>
              {repStats.length>0&&(
                <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:14,marginBottom:14}}>
                    ...
                    <div key={r.rep} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 0',borderBottom:i<repStats.length-1?`1px solid ${C.border}`:'none'}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                          ...
                          <span style={{fontSize:10,color:C.yellow}}>◑{r.progress}</span>
                          <span style={{fontSize:10,color:C.orange}}>↺{r.practice}</span>
          ...
          {!dashLoading&&(!dashData||dashData.error)&&<div style={{textAlign:'center',color:C.gray,padding:'40px 0',fontSize:13}}>Dashboard unavailable. Check connection.</div>}
        </div>
      )}
          ...
          {results.length>0&&(
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:16}}>
              {[{label:'Won',val:won,color:C.green},{label:'Progress',val:progress,color:C.yellow},{label:'Practice',val:practice,color:C.orange}].map(({label,val,color})=>(
                <div key={label} style={{background:`${color}10`,border:`1px solid ${color}33`,borderRadius:10,padding:12,textAlign:'center'}}><div style={{fontFamily:fH,fontSize:30,fontWeight:900,color}}>{val}</div><div style={{fontFamily:fH,fontSize:9,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color}}>{label}</div></div>
              ))}
            </div>
          )}
          {!results.length
      ...
      {activeTab==='log'&&(
        <div>
          <div style={{background:'rgba(26,107,255,0.06)',border:'1px solid rgba(26,107,255,0.18)',borderRadius:8,padding:'10px 14px',marginBottom:14,fontSize:12,color:C.gray,lineHeight:1.6}}>
            Use this to manually log a result from a live customer interaction — when you used a script in real life and want to track the outcome.
          </div>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:14}}>
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              ...
              {isMgr&&<div><div style={{fontFamily:fH,fontSize:10,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:C.gray,marginBottom:5}}>TEAM MEMBER (optional)</div><input style={inp} placeholder="Name" value={form.rep} onChange={e=>setForm(f=>({...f,rep:e.target.value}))}/></div>}
